...
class AsyncQueueEntry{
  constructor(item,callback){
    this.item=item
    this.state=QUEUE_STATE;//默认状态时等待执行
...
class AsyncQueue{
  constructor({name,parallelism,processor,getKey}){
    ...
    this._getKey=getKey;//每个条目的唯一标识获取哈数
  ...
  add(item,callback){
    const key=this.getKey(item);//获取此条目的key
    const entry=this._entries.get(key);//去_entries获取下一个老的条目
    // TODO 有的话如何处理
