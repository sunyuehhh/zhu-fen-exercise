...
class Compilation{
  constructor(options,compiler){
    ...
    this.modules=[];//这里放置本次编译涉及的所有模块
    this.chunks=[];//本次编译所组装出的代码块
    this.assets={};//key是文件名 值是文件内容
    this.files=[];//代表本地打包出来的文件
  ...
  build(callback){
    ...
    console.log(this.fileDependencies,'fileDependencies')
  ...
  buildModule(name,modulePath){
    ...
    // 7.再找到改模块依赖的模块，再递归本步骤知道所有入口依赖的文件都经过了本步骤的处理
    // 声明当前模块的ID
    let moduleId='./'+path.posix.relative(baseDir,modulePath)

    // 创建一个模块ID就是相对于根目录的相对路径  dependencies就是此模块依赖的模块 name是模块所属的代码块的名称
...
function tryExtensions(modulePath,extensions){
  ...
  throw new Error(`${modulePath}找不到`)
